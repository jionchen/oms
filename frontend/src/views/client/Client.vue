<template>
  <div>
    <a-card title="客户管理">
      <a-row gutter="16">
        <a-col :span="8">
          <a-input-search v-model="searchForm.search" placeholder="编号, 名称, 联系人, 电话, 邮箱" allowClear @search="search" />
        </a-col>
        <a-col :span="8">
          <a-space>
        ...
        <a-col :span="8">
          <div style="float: right;">
            <a-button type="primary" icon="plus" @click="openFormModal(form)">新增客户</a-button>
          </div>
        </a-col>
      </a-row>
...
<script>
  import { clientList, clientDestroy, clientExportExcel, clientImportExcel } from '@/api/sales'
  ...
  export default {
    ...
    methods: {
      ...
      exportExcel() {
        exportExcel(clientExportExcel, '客户列表');
      },
      importExcel(item) {
        let data = new FormData();
        data.append('file', item.file);
        clientImportExcel(data)
          .then(() => {
